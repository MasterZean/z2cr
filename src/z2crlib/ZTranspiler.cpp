...
void ZTranspiler::WriteOutro() {
	...
	if (comp.MainFunction->InClass == false) {
		...
		NL();
		cs << "auto temp = new ";
		WFOwnerClsName(*comp.MainFunction, false);
		cs << "()";
		ES();
...
void ZTranspiler::WriteFuncs(Vector<ZFunction*>& Use) {
	...
	for (int j = 0; j < classes.GetCount(); j++) {
		if (classes[j]->IsClass) {
			ZClass& cls = *(ZClass*)classes[j];
					
			NL();
			cs << "namespace ";
			NsIntro(cls.Owner());
			cs << " ";
			cs << "class " << cls.BackName << ";";
			cs << " ";
			NsOutro(cls.Owner());
...
bool ZTranspiler::TranspileClassDeclMaster(ZClass& cls, int accessFlags, bool checkDepends) {
	...
	if (TranspileClassDecl(cls, -1) == 0) {
		...
		if (cls.InUse && !cls.CoreSimple && &cls != ass.CClass && &cls != ass.CRaw && !(cls.FromTemplate/* && cls.TBase != ass.CRaw*/) && !cls.IsStatic)
			NewMember();
	}
	EndClass();
	EndNamespace();
